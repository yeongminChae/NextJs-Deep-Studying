...
import { getTopRatedTv, IGetTvResult } from "../../../../api/tvApi";

interface ISlider {
  SliderTitle: string;
}

export default function SliderTopRatedTv({ SliderTitle }: ISlider) {
  ...
  const onBoxClick = (tvId: number) => {
    router.push(`?tvId=${top_ratedTvData}`, `/nomflix/${tvId} `);
  };
  ...
  const increaseIndex = () => {
    if (top_ratedTvData) {
      ...
      toggleLeaving();
      setBack(false);
      const totalTvs = top_ratedTvData?.results.length - 1;
